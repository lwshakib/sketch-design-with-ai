/**
 * Injects a series of helper scripts, styles, and configurations into the provided HTML string.
 * This is primarily used for the live preview in the canvas, enabling features like:
 * - Google Fonts and Lucide Icons
 * - Tailwind CSS via CDN
 * - Real-time height reporting for the iframe
 * - Element selection and hover highlighting for the editor
 * - Inter-process communication between the iframe and the parent workspace
 *
 * @param html The original HTML content generated by the AI.
 * @returns The HTML content with necessary design system and editor logic injected.
 */
export const getInjectedHTML = (html: string, themeVariables?: any) => {
  // Prevent duplicate injection if the HTML already contains our key script identifier
  if (
  ...
  const scriptLogic = `
    <script id="sketch-injected-script">
      (function() {
        ...
        var updateHistory = [];
        /**
         * Reports the current content height of the iframe back to the parent.
         * Includes loop prevention and smoothing logic to avoid infinite resizing.
         */
        var sendHeight = function() {
          ...
          heightTimeout = setTimeout(function() {
            ...
            var hasViewportRelative = false;
            try {
              if (document.querySelector('.min-h-screen, .h-screen, [class*="h-screen"], [class*="min-h-screen"]')) {
                hasViewportRelative = true;
              }
            } catch (e) {}

            // Fallback to standard measurements but prioritize maxBottom
            var height = maxBottom;
            if (!hasViewportRelative) {
              height = Math.max(
                maxBottom,
                body.scrollHeight,
                html.scrollHeight
        ...
        window.addEventListener('message', function(event) {
          ...
          if (event.data.type === 'UPDATE_THEME') {
            ...
            var typography = vars.typography || {};
            ...
            var isDark = true;
            if (bg.length === 6) {
              var r = parseInt(bg.substring(0, 2), 16);
              var g = parseInt(bg.substring(2, 4), 16);
              var b = parseInt(bg.substring(4, 6), 16);
              var yiq = (r * 299 + g * 587 + b * 114) / 1000;
              isDark = yiq < 128;
            }
            var borderVal = isDark ? "rgba(255, 255, 255, 0.08)" : "rgba(0, 0, 0, 0.08)";
            var cardVal = isDark ? "#141414" : "#ffffff";

            var styleTag = document.getElementById('theme-overrides');
            if (!styleTag) {
...
/**
 * Sanitizes the HTML document for export or storage.
 * Removes all injected editor logic, highlight classes, and internal attributes.
 *
 * @param doc The Document object to sanitize.
 * @param originalHtml The original raw HTML (fallback).
 * @returns A clean HTML string ready for external use.
 */
export const sanitizeDocumentHtml = (doc: Document, originalHtml: string) => {
  ...
  // Remove injected script and style that were added for the internal preview
  ...
  while (current) {
    if (current.style) {
      // Clean up internal editor highlight classes
    ...
    // Clean up contenteditable attributes if any were added during editing
